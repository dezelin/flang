    ...
    *num_events = 0;
    ...
    for (i = 0; i < caml_array_length(events_heap); i++) {
        for (l = Field(events_heap, i); l != Val_int(0); l = Field(l, 1)) {
            ...
            {
                uintnat fnsz = caml_string_length(Field(ev_start, POS_FNAME)) +
                    1;
    ...
    value events_heap)
{
    ...
    if (events_heap == Val_unit) {
        ...
        debug_info->events =
            process_debug_events(code_start, events_heap,
...
void read_main_debug_info(struct debug_info *di)
{
    ...
    if (caml_seek_optional_section(fd, &trail, "DBUG") != -1) {
        ...
        for (i = 0; i < num_events; i++) {
            ...
            caml_input_val(
                chan); /* Skip the list of absolute directory names */
            /* Relocate events in event list */
            for (l = evl; l != Val_int(0); l = Field(l, 1)) {
                value ev = Field(l, 0);
                Field(ev, EV_POS) = Val_long(
                    Long_val(Field(ev, EV_POS)) + orig);
            }
            /* Record event list */
        ...
        di->events = process_debug_events(caml_start_code, events,
            &di->num_events);
    }
