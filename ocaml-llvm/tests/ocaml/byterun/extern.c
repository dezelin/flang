...
static uintnat obj_counter;
/* Number of objects emitted so far */
static uintnat size_32;
/* Size in words of 32-bit block for struct. */
...
{
    value obj;
    /* address of object + initial color in low 2 bits */
...
static void extern_record_location(value obj)
{
    ...
    if (extern_trail_cur == extern_trail_limit) {
        ...
        extern_trail_cur = extern_trail_block->entries;
        extern_trail_limit = extern_trail_block->entries +
            ENTRIES_PER_TRAIL_BLOCK;
    }
    hdr = Hd_val(obj);
        ...
        if (Is_in_value_area(v)) {
            ...
            switch (tag) {
                ...
                case Double_tag: {
                    if (sizeof(double) != 8) {
                        extern_invalid_argument(
                            "output_value: non-standard floats");
                    }
                ...
                default: {
                    ...
                    if (sz > 1) {
                        sp++;
                        if (sp >= extern_stack_limit) {
                            sp = extern_resize_stack(sp);
                        }
                        sp->v = &Field(v, 1);
                        sp->count = sz - 1;
                    }
...
CAMLexport void caml_serialize_block_2(void *data, intnat len)
{
...
#ifndef ARCH_BIG_ENDIAN
    {
        unsigned char *p;
        char *q;
        for (p = data, q = extern_ptr;
            len > 0; len--, p += 2, q += 2) Reverse_16(q, p);
        extern_ptr = q;
    }
#else
...
CAMLexport void caml_serialize_block_4(void *data, intnat len)
{
...
#ifndef ARCH_BIG_ENDIAN
    {
        ...
        char *q;
        for (p = data, q = extern_ptr;
            len > 0; len--, p += 4, q += 4) Reverse_32(q, p);
        extern_ptr = q;
    }
#else
...
CAMLexport void caml_serialize_block_8(void *data, intnat len)
{
...
#ifndef ARCH_BIG_ENDIAN
    {
        ...
        char *q;
        for (p = data, q = extern_ptr;
            len > 0; len--, p += 8, q += 8) Reverse_64(q, p);
        extern_ptr = q;
    }
#else
