...
{
    value filler1;
    /* Make sure the sentinel is never adjacent to any block. */
    header_t h;
    value first_field;
...
static value fl_prev = Fl_head;
/* Current allocation pointer. */
static value fl_last = Val_NULL;
/* Last block in the list.  Only valid
                                 just after [caml_fl_allocate] returns NULL. */
value caml_fl_merge = Fl_head;
/* Current insertion pointer.  Managed
        ...
        case Policy_first_fit: {
...
#endif
                            result = allocate_block(Whsize_wosize (wo_sz),
                                flp_size - 1, prev,
                                cur);
                            goto update_flp;
                        }
            ...
            while (cur != Val_NULL) {
                ...
                if (sz < prevsz) {
                    ...
                    return allocate_block(Whsize_wosize (wo_sz), flp_size, prev,
                        cur);
                }
                prev = cur;
                cur = Next (prev);
            ...
            if (i < flp_size) {
                ...
                if (i == flp_size - 1) {
                    ...
                    if (FLP_MAX >= flp_size + j - 1) {
                        if (j != 1) {
                            memmove(&flp[i + j], &flp[i + 1],
                                sizeof(value) * (flp_size - i - 1));
                        }
                        ...
                        if (FLP_MAX > i + j) {
                            if (j != 1) {
                                memmove(&flp[i + j], &flp[i + 1],
                                    sizeof(value) * (FLP_MAX - i - j));
                            }
                            if (j > 0) {
                                memmove(&flp[i], &buf[0], sizeof(value) * j);
                            }
                        }
                        else {
                            if (i != FLP_MAX) {
                                memmove(&flp[i], &buf[0],
                                    sizeof(value) * (FLP_MAX - i));
                            }
                        }
                        flp_size = FLP_MAX - 1;
