...
static int heap_is_pure;
/* The heap is pure if the only gray objects
...
extern char *caml_fl_merge;
/* Defined in freelist.c. */

static char *markhp, *chunk, *limit;

int caml_gc_subphase;
/* Subphase_{main,weak1,weak2,final} */
            ...
            switch (Color_hd (hd)) {
                case Caml_white:
                    if (Tag_hd (hd) == Custom_tag) {
                        void (*final_fun)(value) = Custom_ops_val(
                            Val_hp(hp))->finalize;
                        if (final_fun != NULL) final_fun(Val_hp(hp));
                    }
                    caml_gc_sweep_hp = (char *) caml_fl_merge_block(
                        Val_hp (hp));
                    break;
                case Caml_blue:
...
static asize_t clip_heap_chunk_size(asize_t request)
{
    ...
    return
        Wsize_bsize (
            ((Bsize_wsize(request) + Page_size - 1) >> Page_log) << Page_log);
}
...
void caml_init_major_heap(asize_t heap_size)
{
    ...
    caml_heap_start = (char *) caml_alloc_for_heap(
