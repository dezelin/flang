...
void read_console_poll(HANDLE hStop, void *_data)
{
    ...
    while (lpSelectData->EState == SELECT_STATE_NONE) {
        ...
        if (check_error(lpSelectData,
            PeekConsoleInput(lpQuery->hFileDescr, &record, 1, &n) == 0)) {
            break;
        }
        ...
        if (record.EventType == KEY_EVENT &&
            ...
            if (check_error(lpSelectData,
                ReadConsoleInput(lpQuery->hFileDescr, &record, 1, &n) == 0)) {
                break;
            }
        }
    };
    ...
    unsigned int uFlagsFd)
{
    ...
    select_data_query_add(res, SELECT_MODE_READ, hFileDescr, lpOrigIdx,
        uFlagsFd);
...
void socket_poll(HANDLE hStop, void *_data)
{
    ...
    DEBUG_PRINT("Worker has %d queries to service",
        lpSelectData->nQueriesCount);
    ...
    if (lpSelectData->nError == 0) {
        for (i = 0; i < lpSelectData->nQueriesCount; i++) {
                    ...
                    if (WSAEnumNetworkEvents((SOCKET)(iterQuery->hFileDescr),
                        ...
                        if ((iterQuery->EMode & SELECT_MODE_READ) != 0 &&
                            ...
                            select_data_result_add(lpSelectData,
                                SELECT_MODE_READ, iterQuery->lpOrigIdx);
                        }
                        if ((iterQuery->EMode & SELECT_MODE_WRITE) != 0 &&
                            (events.lNetworkEvents &
                                (FD_WRITE | FD_CONNECT | FD_CLOSE)) != 0) {
                            select_data_result_add(lpSelectData,
                                SELECT_MODE_WRITE, iterQuery->lpOrigIdx);
                        }
                        if ((iterQuery->EMode & SELECT_MODE_EXCEPT) != 0 &&
                            (events.lNetworkEvents & FD_OOB) != 0) {
                            select_data_result_add(lpSelectData,
                                SELECT_MODE_EXCEPT, iterQuery->lpOrigIdx);
                        }
                    }
                }
            }
            ...
            if (iterQuery->uFlagsFd & FLAGS_FD_IS_BLOCKING) {
                ...
                iMode = 0;
                check_error(lpSelectData,
                    WSAEventSelect((SOCKET)(iterQuery->hFileDescr), aEvents[i],
                        0) != 0 ||
                        ioctlsocket((SOCKET)(iterQuery->hFileDescr), FIONBIO,
                            &iMode) != 0);
            }
            else {
                check_error(lpSelectData,
    ...
    unsigned int uFlagsFd)
{
    ...
    /* Polling socket can be done mulitple handle at the same time. You just
       ...
       */
    DEBUG_PRINT(
        "Scanning list of worker to find one that already handle socket");
    /* Search for job */
    ...
    if (res == NULL) {
        ...
        /* No matching job found, create one */
        if (res == NULL) {
            DEBUG_PRINT("No job for type %d found, create one",
                SELECT_TYPE_SOCKET);
    ...
    switch (get_handle_type(fd)) {
        ...
        case SELECT_HANDLE_PIPE:
            ...
            if (EMode == SELECT_MODE_READ) {
                ...
                res = read_pipe_poll_add(res, EMode, hFileDescr, lpOrigIdx,
                    uFlagsFd);
            }
            else if (EMode == SELECT_MODE_WRITE) {
                DEBUG_PRINT(
                    "No need to check availability of data on pipe, write operation always possible");
                res = static_poll_add(res, EMode, hFileDescr, lpOrigIdx,
                    uFlagsFd);
            };
            break;
        ...
        case SELECT_HANDLE_SOCKET:
            ...
            if (getsockname((SOCKET) hFileDescr, &sa, &sa_len) ==
                SOCKET_ERROR) {
                if (WSAGetLastError() == WSAEINVAL) {
                    /* Socket is not bound */
                    DEBUG_PRINT("Socket is not connected");
                    if (EMode == SELECT_MODE_WRITE ||
                        EMode == SELECT_MODE_READ) {
                        res = static_poll_add(res, EMode, hFileDescr, lpOrigIdx,
                            uFlagsFd);
                        alreadyAdded = TRUE;
                    }
            ...
            if (!alreadyAdded) {
                res = socket_poll_add(res, EMode, hFileDescr, lpOrigIdx,
                    uFlagsFd);
            }
            break;
...
if (
fdlist_to_fdset(readfds,
&read) &&
fdlist_to_fdset(writefds,
&write) &&
fdlist_to_fdset(exceptfds,
&except)) {
...
{
DEBUG_PRINT("Discarding handle %x which is already monitor for read",
    Handle_val(fd));
}
}
...
{
DEBUG_PRINT("Discarding handle %x which is already monitor for write",
    Handle_val(fd));
}
}
handle_set_reset(&hds);
...
{
DEBUG_PRINT("Discarding handle %x which is already monitor for exceptional",
    Handle_val(fd));
}
}
handle_set_reset(&hds);
...
worker_job_event_done(iterSelectData
->lpWorker);
nEventsCount++;
};
...
switch (
WaitForMultipleObjects(nEventsCount, lpEventsDone, FALSE, milliseconds
))
{
case
WAIT_FAILED
...
{
worker_job_stop(iterSelectData
->lpWorker);
};
iterSelectData = LIST_NEXT(LPSELECTDATA, iterSelectData);
};
...
switch (
WaitForMultipleObjects(nEventsCount, lpEventsDone, TRUE, INFINITE
))
{
case
WAIT_FAILED
...
Store_field(l,
0,
find_handle(iterResult, readfds, writefds, exceptfds
));
switch (iterResult->EMode)
{
case
