...
zero.
HighPart = zero.LowPart = 0;
set_file_pointer(h, zero,
&cur_position, FILE_CURRENT);
...
QuadPart = -1;
overlap.
OffsetHigh = cur_position.HighPart;
overlap.
Offset = cur_position.LowPart;
}
else if(l_len > 0) {
...
beg_position.
QuadPart = cur_position.QuadPart - lock_len.QuadPart;
overlap.
OffsetHigh = beg_position.HighPart;
overlap.
Offset = beg_position.LowPart;
}
...
/* F_LOCK - blocking write lock */
enter_blocking_section();

if (!
LockFileEx(h, LOCKFILE_EXCLUSIVE_LOCK,
0,
lock_len.LowPart, lock_len.HighPart, &overlap))
err = GetLastError();
...
if (!
LockFileEx(h, LOCKFILE_FAIL_IMMEDIATELY
...
lock_len.LowPart, lock_len.HighPart, &overlap)) {
UnlockFileEx(h,
0, lock_len.LowPart, lock_len.HighPart, &overlap);
} else {
err = GetLastError();
}
break;
