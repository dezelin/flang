...
BitBlt(hdc,
0,0,grwindow.width,grwindow.height,
...
grwindow.
hwnd = CreateWindow(szOcamlWindowClass,
    WINDOW_NAME,
    WS_OVERLAPPEDWINDOW,
    x, y,
    w, h,
...
CAMLprim value
caml_gr_open_graph(value
arg)
{
long tid;
if (gr_initialized) return
Val_unit;
...
CAMLprim value
caml_gr_close_graph(value
unit)
{
if (gr_initialized) {
PostMessage(grwindow
.hwnd, WM_CLOSE, 0, 0);
...
if(grremember_mode) {
BitBlt(grwindow
.gcBitmap,0,0,grwindow.width,grwindow.height,
grwindow.gcBitmap,0,0,WHITENESS);
}
if(grdisplay_mode) {
BitBlt(grwindow
.gc,0,0,grwindow.width,grwindow.height,
...
void gr_fail(char *fmt, char *arg)
{
    ...
    if (graphic_failure_exn == NULL) {
        ...
        if (graphic_failure_exn == NULL) {
            invalid_argument(
                "Exception Graphics.Graphic_failure not initialized, must link graphics.cma");
        }
    }
