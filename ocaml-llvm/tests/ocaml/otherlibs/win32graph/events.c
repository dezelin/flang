...
static unsigned int caml_gr_tail = 0;
...
void caml_gr_handle_event(UINT msg, WPARAM wParam, LPARAM lParam)
{
    switch (msg) {
        ...
        case WM_CHAR:
            caml_gr_enqueue_event(EVENT_KEY_PRESSED, last_pos, last_button,
                wParam);
            break;
...
caml_gr_wait_event_poll();

else
return
caml_gr_wait_event_blocking(mask);
}
