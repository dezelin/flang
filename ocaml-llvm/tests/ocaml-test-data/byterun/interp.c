...
/* Function application */
            ...
            Instruct(GRAB): {
                ...
                if (extra_args >= required) {
                    ...
                    Code_val(accu) = pc -
                        3; /* Point to the preceding RESTART instr. */
                    sp += num_args;
                    pc = (code_t) (sp[0]);
                    env = sp[1];
                    extra_args = Long_val(sp[2]);
            ...
            Instruct(CLOSUREREC): {
                ...
                *--sp = accu;
                p++;
                for (i = 1; i < nfuncs; i++) {
                    *p = Make_header(i * 2, Infix_tag,
                        Caml_white);  /* color irrelevant. */
                    p++;
                    *p = (value) (pc + pc[i]);
                    *--sp = (value) p;
                    p++;
...
/* Allocation of blocks */
            ...
            Instruct(MAKEFLOATBLOCK): {
                ...
                if (size <= Max_young_wosize / Double_wosize) {
                    ...
                    block = caml_alloc_shr(size * Double_wosize,
                        Double_array_tag);
                }
...
/* Exceptions */
            ...
            Instruct(RERAISE):
                ...
                if (caml_backtrace_active) {
                    caml_stash_backtrace(accu, pc, sp, 1);
                }
                goto raise_notrace;
            ...
            raise_exception:
                ...
                if (caml_backtrace_active) {
                    caml_stash_backtrace(accu, pc, sp, 0);
                }
            raise_notrace:
                if ((char *) caml_trapsp
