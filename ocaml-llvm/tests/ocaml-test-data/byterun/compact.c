...
static char *compact_allocate(mlsize_t size)
{
    ...
    while (
        Chunk_size (compact_fl) - Chunk_alloc (compact_fl) <= Bhsize_wosize (3)
...
static void do_compaction(void)
{
                    ...
                    *p = Make_header (Wosize_whsize(sz), t, Caml_white);
                    ...
                    if (infixes != NULL) {
                        ...
                        while (Ecolor ((word) infixes) != 3) {
                            ...
                            *infixes = Make_header (infixes - p, Infix_tag,
                                Caml_white);
                            infixes = (word *) q;
                        }
                    }
    ...
    /* Shrink the heap if needed. */
    {
        ...
        ch = caml_heap_start;
        while (ch != NULL) {
            char *next_chunk = Chunk_next (
                ch);  /* Chunk_next (ch) will be erased */
...
/* used in gc_ctrl.c and memory.c */
