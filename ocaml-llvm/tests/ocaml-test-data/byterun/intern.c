            ...
            case OReadItems:
                ...
                if (code >= PREFIX_SMALL_INT) {
                    ...
                    if (code >= PREFIX_SMALL_STRING) {
                        ...
                        *intern_dest = Make_header(size, String_tag,
                            intern_color);
                        intern_dest += 1 + size;
                        Field(v, size - 1) = 0;
                        ...
                        switch (code) {
                            ...
                            case CODE_DOUBLE_BIG:
                                ...
                                *intern_dest = Make_header(Double_wosize,
                                    Double_tag, intern_color);
                            ...
                            read_double_array:
                                ...
                                *intern_dest = Make_header(size,
                                    Double_array_tag, intern_color);
                                intern_dest += 1 + size;
                            ...
                            case CODE_CODEPOINTER:
                                ofs = read32u();
                                readblock(digest, 16);
                                codeptr = intern_resolve_code_pointer(digest,
                                    ofs);
                                if (codeptr != NULL) {
                                    v = (value) codeptr;
                                }
                            ...
                            case CODE_CUSTOM:
                                ops = caml_find_custom_operations(
                                    (char *) intern_src);
                                if (ops == NULL) {
                                    intern_cleanup();
                                    caml_failwith(
                                ...
                                size = ops->deserialize(
                                    (void *) (intern_dest + 2));
                                size = 1 +
                                    (size + sizeof(value) - 1) / sizeof(value);
                                v = Val_hp(intern_dest);
                                if (intern_obj_table != NULL)
                                    intern_obj_table[obj_counter++] = v;
                                *intern_dest = Make_header(size, Custom_tag,
                                ...
                                if (ops->finalize != NULL && Is_young(v)) {
                                    ...
                                    if (caml_finalize_table.ptr >=
                                        caml_finalize_table.limit) {
                                        CAMLassert (caml_finalize_table.ptr ==
                                            caml_finalize_table.limit);
...
static void intern_alloc(mlsize_t whsize, mlsize_t num_objects)
{
    ...
    obj_counter = 0;
    if (num_objects > 0) {
        intern_obj_table = (value *) caml_stat_alloc(
            num_objects * sizeof(value));
    }
    else {
        intern_obj_table = NULL;
    }
    ...
    asize_t offset)
{
    ...
    for (i = caml_code_fragments_table.size - 1; i >= 0; i--) {
        ...
        if (!cf->digest_computed) {
            caml_md5_block(cf->digest, cf->code_start,
...
CAMLexport void caml_deserialize_block_2(void *data, intnat len)
{
    ...
    unsigned char *p, *q;
    for (p = intern_src, q = data; len > 0; len--, p += 2, q += 2) Reverse_16(q,
        p);
    intern_src = p;
#else
...
#ifndef ARCH_BIG_ENDIAN
    unsigned char *p, *q;
    for (p = intern_src, q = data; len > 0; len--, p += 4, q += 4) Reverse_32(q,
        p);
    intern_src = p;
#else
...
#ifndef ARCH_BIG_ENDIAN
    unsigned char *p, *q;
    for (p = intern_src, q = data; len > 0; len--, p += 8, q += 8) Reverse_64(q,
        p);
    intern_src = p;
#else
