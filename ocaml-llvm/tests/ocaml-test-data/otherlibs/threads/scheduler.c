...
struct caml_thread_struct
{
    ...
    value *trapsp;
    value backtrace_pos;
    /* The backtrace info for this thread */
    backtrace_slot *backtrace_buffer;
    value backtrace_last_exn;
    value status;
    /* RUNNABLE, KILLED. etc (see below) */
    value fd;
    /* File descriptor on which we're doing read or write */
    value readfds, writefds, exceptfds;
    /* Lists of file descriptors on which we're doing select() */
    value delay;
    /* Time until which this thread is blocked */
    value joining;
    /* Thread we're trying to join */
    value waitpid;
    /* PID of process we're waiting for */
...
static value schedule_thread(void)
{
    ...
    if (need_select || run_thread == NULL) {
        ...
        retcode = select(FD_SETSIZE, &readfds, &writefds, &exceptfds,
            delay_ptr);
