...
static value gr_draw_or_fill_arc(value
vx,
value vy, value
vrx,
value vry,
...
if(grremember_mode)
SetPixel(grwindow
.gcBitmap, x, Wcvt(y),grwindow.CurrentColor);
if(grdisplay_mode) {
SetPixel(grwindow
.gc, x, Wcvt(y),grwindow.CurrentColor);
}
return
Val_unit;
}
...
if(grremember_mode)
MoveToEx(grwindow
.gcBitmap,grwindow.grx,Wcvt(grwindow.gry),0);
if (grdisplay_mode)
MoveToEx(grwindow
.gc,grwindow.grx,Wcvt(grwindow.gry),0);
return
Val_unit;
}

CAMLprim value
caml_gr_current_x(value
unit)
{
return
Val_int(grwindow
...
SelectObject(grwindow
.gc,grwindow.CurrentPen);
SelectObject(grwindow
.gcBitmap,grwindow.CurrentPen);
if (grremember_mode)
LineTo(grwindow
.gcBitmap,x,Wcvt(y));
if (grdisplay_mode)
...
SetBkMode(grwindow
.gcBitmap,TRANSPARENT);
SetTextAlign(grwindow
.gcBitmap, TA_UPDATECP|TA_BOTTOM);
SetTextAlign(grwindow
.gc, TA_UPDATECP|TA_BOTTOM);
if (grremember_mode) {
TextOut(grwindow
.gcBitmap,0,0,(char *)text,x);
}
if(grdisplay_mode) {
TextOut(grwindow
.gc,0,0,(char *)text,x);
}
GetCurrentPosition(grwindow
.gc,&pt);
grwindow.
grx = pt.x;
grwindow.
gry = grwindow.height - pt.y;
SetBkMode(grwindow
.gc,oldmode);
SetBkMode(grwindow
.gcBitmap,oldmode);
...
if (grdisplay_mode)
FillRect(grwindow
.gc,&rc,grwindow.CurrentBrush);
if (grremember_mode)
FillRect(grwindow
.gcBitmap,&rc,grwindow.CurrentBrush);
return
Val_unit;
}
...
bottom = top + radius;
Ellipse(grwindow
.gcBitmap,left,top,right,bottom);
return
Val_unit;
}

CAMLprim value
caml_gr_set_window_title(value
text)
{
SetWindowText(grwindow
.hwnd,(char *)text);
return
Val_unit;
}
...
CAMLprim value
caml_gr_draw_arc(value
*argv,
int argc
)
{
return
gr_draw_or_fill_arc(argv[0], argv[1], argv[2], argv[3],
    argv[4], argv[5], FALSE
);
}

CAMLprim value
caml_gr_draw_arc_nat(vx, vy, vrx, vry, vstart, vend
)
{
return
gr_draw_or_fill_arc(vx, vy, vrx, vry, vstart, vend, FALSE
);
}

CAMLprim value
caml_gr_set_line_width(value
vwidth)
{
...
SelectObject(grwindow
.gc,newPen);
DeleteObject(grwindow
.CurrentPen);
grwindow.
CurrentPen = newPen;
SetTextColor(grwindow
.gc,c);
SetTextColor(grwindow
.gcBitmap,c);
...
.gcBitmap,grwindow.CurrentPen);
SelectObject(grwindow
.gcBitmap,grwindow.CurrentBrush);
if( fill )
Pie(grwindow
.gcBitmap,x1, Wcvt(y1), x2, Wcvt(y2),
...
.gc,grwindow.CurrentPen);
SelectObject(grwindow
.gc,grwindow.CurrentBrush);
if (fill)
Pie(grwindow
.gc,x1, Wcvt(y1), x2, Wcvt(y2),
x3, Wcvt(y3), x4, Wcvt(y4));
else
Arc(grwindow
.gc,x1, Wcvt(y1), x2, Wcvt(y2),
...
CAMLprim value
caml_gr_show_bitmap(value
filename,
int x,
int y
)
{
AfficheBitmap(filename, grwindow
.gcBitmap,x,Wcvt(y));
AfficheBitmap(filename, grwindow
.gc,x,Wcvt(y));
return
...
CAMLprim value
caml_gr_get_mousex(value
unit)
{
POINT pt;
GetCursorPos(&pt);
MapWindowPoints(HWND_DESKTOP, grwindow
.hwnd,&pt,1);
return pt.
x;
}
...
i++ ){
p->
x = Int_val(Field(Field(vect, i), 0));
p->
y = Wcvt(Int_val(Field(Field(vect, i), 1)));
p++;
}
if (grremember_mode) {
SelectObject(grwindow
...
CAMLprim value
caml_gr_fill_arc_nat(vx, vy, vrx, vry, vstart, vend
)
{
return
gr_draw_or_fill_arc(vx, vy, vrx, vry, vstart, vend, TRUE
);
}
...
static void finalize_image(value
i)
{
DeleteObject (Data(i));
...
BitBlt(grwindow
.gcBitmap,xdst, ydst, Width(i), Height(i),
