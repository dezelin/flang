...
static unsigned int caml_gr_head = 0;
    ...
    switch (event->type) {
        ...
        case ConfigureNotify:
            ...
            if (caml_gr_window.w > caml_gr_bstore.w
                ...
                newbstore.win =
                    XCreatePixmap(caml_gr_display, caml_gr_window.win,
                        newbstore.w,
                        newbstore.h,
                        XDefaultDepth(caml_gr_display, caml_gr_screen));
                newbstore.gc = XCreateGC(caml_gr_display, newbstore.win, 0,
                    NULL);
        ...
        case ButtonPress:
        case ButtonRelease:
            caml_gr_enqueue_event(event->type, event->xbutton.x,
                event->xbutton.y,
                event->type == ButtonPress, 0);
            break;

        case MotionNotify:
            caml_gr_enqueue_event(event->type, event->xmotion.x,
                event->xmotion.y,
