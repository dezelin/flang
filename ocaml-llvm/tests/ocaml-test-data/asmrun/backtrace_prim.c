...
frame_descr *caml_next_frame_descriptor(uintnat *pc, char **sp)
{
    ...
    while (1) {
        ...
        while (1) {
            ...
            if (d == NULL) {
                return NULL;
            } /* happens if some code compiled without -g */
...
void caml_stash_backtrace(value exn, uintnat pc, char *sp, char *trapsp)
{
    ...
    if (caml_backtrace_buffer == NULL) {
        ...
        caml_backtrace_buffer = malloc(
            BACKTRACE_BUFFER_SIZE * sizeof(backtrace_slot));
...
CAMLprim value
caml_get_current_callstack(value
max_frames_value)
{
...
for (
trace_pos = 0;
trace_pos<trace_size;
trace_pos++) {
frame_descr *descr = caml_next_frame_descriptor(&pc, &sp);
Assert(descr
!= NULL);
Store_field(trace, trace_pos, caml_val_raw_backtrace_slot((backtrace_slot)
descr));
}
}
...
void caml_extract_location_info(backtrace_slot slot, /*out*/
...
CAMLprim value
caml_add_debug_info(backtrace_slot
start,
value size, value
events)
{
...
CAMLprim value
caml_remove_debug_info(backtrace_slot
start)
{
return
Val_unit;
